"""
Generate TensorZero 简易使用文档.docx from the companion Markdown file.

Reads:  TensorZero简易使用文档.md (same directory)
Writes: TensorZero简易使用文档.docx (same directory)

Supports:
  - headings (#, ##, ###, ####)
  - paragraphs with inline `code`, **bold**, [text](url) links
  - fenced code blocks (```...```)
  - tables (| ... |)
  ...
  - horizontal rules (---)
  - images (![alt](path)) - resolves relative paths to script directory
...
DOCX_PATH = os.path.join(HERE, 'TensorZero简易使用文档.docx')
...
LINK_RE = re.compile(r'(?<!!)\[([^\]]+)\]\(([^)]+)\)')  # negative lookbehind to skip images
...
def add_image(path: str, alt: str = ''):
    """Insert an image. Resolves relative paths against script directory."""
    abs_path = path if os.path.isabs(path) else os.path.join(HERE, path)
    if not os.path.exists(abs_path):
        # Fall back to a placeholder paragraph so the doc still builds.
        p = doc.add_paragraph()
        run = p.add_run(f'[图片缺失: {path}]')
    ...
    run = p.add_run()
    # Cap at 14cm wide (page is ~16cm useful), python-docx auto-keeps aspect.
...
# ──────────────────────────────────────────────────────────────────────────
# Markdown parser (line-based, tailored for this document)
# ──────────────────────────────────────────────────────────────────────────
...
def render_markdown(md: str):
    ...
    while i < n:
        ...
        # Blockquote / callout (consecutive `> ` lines collapsed)
