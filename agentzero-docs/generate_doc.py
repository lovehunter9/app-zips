"""
Generate AgentZero 简易使用文档.docx from the companion Markdown file.

Reads:  AgentZero简易使用文档.md (same directory)
Writes: AgentZero简易使用文档.docx (same directory)
...
MD_PATH = os.path.join(HERE, 'AgentZero简易使用文档.md')
DOCX_PATH = os.path.join(HERE, 'AgentZero简易使用文档.docx')
...
def add_numbered(text: str, num: int, level: int = 0):
    """Render a numbered item with manual prefix.

    We don't use Word's built-in 'List Number' style because it shares a single
    numbering definition across all numbered paragraphs, so a new list in a
    new section continues counting from the previous one (e.g. 4, 5, 6 instead
    of 1, 2, 3). Manual numbering keeps each list's count true to the source.
    """
    p = doc.add_paragraph()
    p.paragraph_format.left_indent = Cm(0.75 * (level + 1))
...
def render_markdown(md: str):
    ...
    while i < n:
        ...
        # Numbered list — preserve source number literally so each list starts
        # at whatever its first item says (typically 1).
