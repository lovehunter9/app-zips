// Audio Minutes X2 Demo — Feishu-Minutes-style app.
...
function SettingsPage({
  ...
  const [caps, setCaps] = useState<Record<string, ModelOpt[]>>({});
  ...
  // Model dropdown. Pass `cap` for audio capabilities (stt/align/diar/enhance/
  // translate) → filtered from `caps` (supports-based); pass `mode` for chat/
  // embedding (摘要/问答/RAG) → filtered from `modes` (mode-based).
  const ModelSelect = ({ label, cap, mode, value, set }: { label: string; cap?: string; mode?: string; value: string; set: (v: string) => void }) => {
    const opts = (cap ? caps[cap] : mode ? modes[mode] : []) || [];
    const tag = cap ? `cap=${cap}` : `mode=${mode}`;
    return (
      <label className="block text-sm">
        <span className="mb-0.5 block text-xs text-neutral-400">{label} <span className="text-neutral-600">({tag})</span></span>
        <select className="input !py-1.5" value={value} onChange={(e) => set(e.target.value)}>
          <option value="">{opts.length ? "— 请选择 —" : "(网关无此模型 — 先加载模型)"}</option>
          {opts.map((o) => (
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-3 border-b border-neutral-800 px-4 py-2.5">
        ...
        {config.version?.label && (
          <span
            className="rounded bg-neutral-800 px-2 py-0.5 font-mono text-[11px] text-neutral-400 ring-1 ring-neutral-700"
            title="当前运行的构建：文件夹 · 最后修改时间 · 版本标记(APP_VERSION)"
          >
            {config.version.label}
          </span>
        )}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        <div className="mx-auto grid w-full max-w-6xl auto-rows-min gap-2.5 md:grid-cols-2 xl:grid-cols-3">
          ...
          <div className="space-y-1.5 rounded-lg border border-neutral-800 bg-neutral-900/50 p-2.5">
            ...
            <ModelSelect label="转写 STT" cap="stt" value={stt} set={setStt} />
            <ModelSelect label="强制对齐 Align" cap="align" value={align} set={setAlign} />
            <ModelSelect label="说话人分离 Diarize" cap="diar" value={diar} set={setDiar} />
          </div>
          ...
          <div className="space-y-1.5 rounded-lg border border-neutral-800 bg-neutral-900/50 p-2.5">
            ...
            <ModelSelect label="翻译模型" cap="translate" value={trModel} set={setTrModel} />
          ...
          <div className="space-y-1.5 rounded-lg border border-neutral-800 bg-neutral-900/50 p-2.5">
            ...
            <ModelSelect label="增强模型" cap="enhance" value={enhModel} set={setEnhModel} />
          </div>
    ...
    <div className="flex h-screen flex-col">
      ...
      <header className="flex items-center gap-3 border-b border-neutral-800 px-4 py-3">
        <button
          ...
          title="返回文件列表"
        >
          🎙 Audio Minutes <span className="text-neutral-500">X2 Demo</span>
        </button>
        <div className="ml-auto flex items-center gap-2">
          <button
            className="btn-ghost"
